...
#
ETH=/etc/sysconfig/network-scripts/ifcfg-`cat /proc/net/dev |grep -v "lo"| awk '{i++; if(i>2){print $1}}' | sed 's/^[\t]*//g' | sed 's/[:]*$//g'`;chmod +x $ETH
echo -e "\033[33m (当前系统默认DNS1为：`cat /etc/resolv.conf|grep -v "# Generated by NetworkManager"|sed -n '1p'| awk '/ /{print $2}'`；\
；DNS2为：`cat /etc/resolv.conf|grep -v "# Generated by NetworkManager"|sed -n '2p'| awk '/ /{print $2}'`\
；若需更改服务器DNS请输入1按回车，或直接按回车跳过！)请输入：\033[0m"&&read num
if [[ "$num" -eq "1" ]];then
    echo -e "\033[33m请输入DNS1：\033[0m"&&read dns1
    echo -e "\033[33m请输入DNS2：\033[0m"&&read dns2
    if cat $ETH | grep "DNS1=" > /dev/null ;then
       sed -i "s/DNS1=.*/DNS1=$dns1/g" $ETH
    else
        echo DNS1=$dns1>>$ETH
    fi
    if cat $ETH | grep "DNS2=" > /dev/null ;then
       sed -i "s/DNS2=.*/DNS2=$dns2/g" $ETH
    else
       echo DNS2=$dns2>>$ETH
    fi
else
    echo -e "\033[33m 将继续使用系统默认DNS1：`cat /etc/resolv.conf|grep -v "# Generated by NetworkManager"|sed -n '1p'| awk '/ /{print $2}'`\
    DNS2：`cat /etc/resolv.conf|grep -v "# Generated by NetworkManager"|sed -n '2p'| awk '/ /{print $2}'`\033[0m"
fi
